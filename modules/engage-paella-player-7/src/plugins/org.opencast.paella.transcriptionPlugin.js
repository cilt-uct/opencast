...
export default class transcriptionPlugin extends PopUpButtonPlugin {
  ...
  async isEnabled() {
    ...
    this._captions = tracks.find(track => track.mimetype === 'text/vtt'
      && (track.type === 'captions/source' || track.type === 'captions/vtt')) ||
    ...
    if (this._transcriptFeaturesAttachment) {
      try {
        ...
        console.warn('Failed to load transcription JSON:', e);
      }
    }

    const hasCaptions = !!this._captions;
    const hasNibityFeatures = !!(this._transcriptFeaturesAttachment && this._transcriptFeatures.length > 0);

    return hasCaptions || hasNibityFeatures;
  ...
  createTabContent() {
    const aiModelText = `
      <div class="ai-model-text">
        <span class='ai-model'>AI-generated transcript ${this._isWhisper ? '' : ' and summaries'}. Click 
  ...
  getInfoContent() {
    ...
    const infoText = `
      ...
      <p>
      ${this._isWhisper ? `This transcript is generated by OpenAI Whisper. While we aim for accuracy,
        the content may be inaccurate or incomplete. If in doubt, watch the full video, or consult your
        lecturer or course material.` : `This transcription is generated by WayWithWords using the AI model
        <strong>${this._transcriptionModel}</strong>. While we aim for accuracy, the content may be inaccurate
        or incomplete. If in doubt, watch the full video, or consult your lecturer or course material.`}
      </p>
      <hr/>`;
