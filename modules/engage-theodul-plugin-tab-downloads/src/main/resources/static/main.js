...
define(['require', 'jquery', 'underscore', 'backbone', 'engage/core', 'moment'], function(require, $, _, Backbone, Engage, Moment) {
  ...
  function initTranslate(language, funcSuccess, funcError) {
    ...
    if (language == 'de') {
      Engage.log('Tab:Downloads: Choosing german translations');
      jsonstr += 'language/de.json';
    } else { // No other languages supported, yet
      Engage.log('Tab:Downloads: Choosing english translations');
  ...
  function getDownloadList(model) {
    ...
    _.each(model.get('tracks'), function(item) {
      if (!Object.prototype.hasOwnProperty.call(item, 'transport')
          && item.url.indexOf('rtmp') == -1
          && item.url.indexOf('flv') === -1) {
  ...
  function getCaptionList(model) {
    ...
    _.each(model.get('attachments'), function(item) {
      ...
      if (item.type.indexOf('captions') >= 0
  ...
  var DownloadsTabView = Backbone.View.extend({
    ...
    render: function() {
      if (!mediapackageError) {
        ...
        if (mediaSeries) {
          mediaSeries = (mediaSeries.indexOf(',') > -1 ?
            mediaSeries.substring(0, mediaSeries.indexOf(',')) : mediaSeries);
        }

        // Construct a download URL
        downloadURL =  mediaSeries ? (mediaSeries + '_' + mediaDate) : mediaDate;
        downloadURL = ('/download/' + downloadURL).replace(/\//g, '_').replace(/\s/g, '_');
        var tempVars = {
