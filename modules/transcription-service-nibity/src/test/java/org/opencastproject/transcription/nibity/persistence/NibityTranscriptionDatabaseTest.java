 ...
 * @author franck
 */
public class NibityTranscriptionDatabaseTest {
  ...
  private static final Date EXPECTED = new Date();
  ...
  public void setUp() throws Exception {
    ...
    database.setEntityManagerFactory(
            newTestEntityManagerFactory("org.opencastproject.transcription.nibity.persistence"));
  ...
  public void testStoreJobControl() throws Exception {
    ...
    database.storeJobControl(MP_ID, TRACK_ID, JOB_ID, STATUS, TRACK_DURATION, EXPECTED, PROVIDER);
  ...
  public void testFindByMediaPackage() throws Exception {
    database.storeJobControl(MP_ID, TRACK_ID, JOB_ID, STATUS, TRACK_DURATION, EXPECTED, PROVIDER);
    database.storeJobControl(MP_ID, TRACK_ID2, JOB_ID2, STATUS, TRACK_DURATION, EXPECTED, PROVIDER);
    database.storeJobControl("another_mp_id", "track3", "job3", STATUS, TRACK_DURATION, EXPECTED, PROVIDER);

    List<NibityTranscriptionJobControl> list = database.findByMediaPackage(MP_ID);
  ...
  public void testFindByOneStatus() throws Exception {
    database.storeJobControl(MP_ID, TRACK_ID, JOB_ID, STATUS, TRACK_DURATION, EXPECTED, PROVIDER);
    database.storeJobControl(MP_ID2, TRACK_ID2, JOB_ID2, STATUS2, TRACK_DURATION, EXPECTED, PROVIDER);
    database.storeJobControl(MP_ID3, TRACK_ID3, JOB_ID3, STATUS, TRACK_DURATION, EXPECTED, PROVIDER);

    List<NibityTranscriptionJobControl> list = database.findByStatus(STATUS);
    Assert.assertEquals(2, list.size());
  ...
  public void testFindByManyStatus() throws Exception {
    ...
    List<NibityTranscriptionJobControl> list = database.findByStatus(STATUS, STATUS2);
  ...
  public void testUpdateJobControlToTranscriptionComplete() throws Exception {
    ...
    Assert.assertEquals(MP_ID, j.getMediaPackageId());
    Assert.assertEquals(TRACK_ID, j.getTrackId());
    Assert.assertEquals(NibityTranscriptionJobControl.Status.TranscriptionComplete.name(), j.getStatus());
