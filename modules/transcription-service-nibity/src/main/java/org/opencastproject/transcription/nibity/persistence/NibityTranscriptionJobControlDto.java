...
public class NibityTranscriptionJobControlDto implements Serializable {
  @Id
  ...
  public NibityTranscriptionJobControlDto() {
  }
  ...
  public static NibityTranscriptionJobControlDto store(EntityManager em, String mediaPackageId, String trackId,
    ...
    try {
      ...
      throw new NibityTranscriptionDatabaseException(e);
    } finally {
      em.close();
    }
  }
  ...
  public static NibityTranscriptionJobControlDto findByJob(EntityManager em, String jobId)
          throws NibityTranscriptionDatabaseException {
    Query query = null;
    try {
      query = em.createNamedQuery("TranscriptionJobControl.findByJob");
      query.setParameter("transcriptionJobId", jobId);
      return (NibityTranscriptionJobControlDto) query.getSingleResult();
  ...
  public static List<NibityTranscriptionJobControlDto> findByMediaPackage(EntityManager em, final String mediaPackageId)
          throws NibityTranscriptionDatabaseException {
    Query query = null;
    try {
  ...
  @SuppressWarnings("unchecked")
  public static List<NibityTranscriptionJobControlDto> findByStatus(EntityManager em, final String... status)
          throws NibityTranscriptionDatabaseException {
  ...
  /** Update job status */
  public static void updateStatus(EntityManager em, String jobId, String status) throws NibityTranscriptionDatabaseException {
    NibityTranscriptionJobControlDto dto = findByJob(em, jobId);
    if (dto == null)
      throw new NibityTranscriptionDatabaseException("Job not found in database: " + jobId);

    EntityTransaction tx = em.getTransaction();
    try {
      tx.begin();
      dto.setStatus(status);
      if (NibityTranscriptionJobControl.Status.TranscriptionComplete.name().equals(status))
  ...
  public static void delete(EntityManager em, String jobId) throws NibityTranscriptionDatabaseException {
    NibityTranscriptionJobControlDto dto = findByJob(em, jobId);
    if (dto == null)
      return;
