...
@Component(
    ...
    property = {
        "service.description=Start Transcription Workflow Operation Handler (Whisper API)",
...
public class WhisperStartTranscriptionOperationHandler extends AbstractWorkflowOperationHandler {
  ...
  static final String RESPONSEFORMAT = "responseformat";
  ...
  public WorkflowOperationResult start(final WorkflowInstance workflowInstance, JobContext context)
    ...
    String model = StringUtils.trimToEmpty(operation.getConfiguration(MODEL));
    String responseFormat = StringUtils.trimToEmpty(operation.getConfiguration(RESPONSEFORMAT));
    // String prompt = StringUtils.trimToEmpty(operation.getConfiguration(PROMPT));
    ...
    for (Track t : mediaPackage.getTracks()) {
      logger.info("Track found: flavor={}, hasAudio={}, uri={}",
          t.getFlavor(), t.hasAudio(), t.getURI());
    }

    /*logger.info("Track found: flavor={}, tags={}, hasAudio={}, uri={}",
        t.getFlavor(),
        Arrays.toString(t.getTags()),
        t.hasAudio(),
        t.getURI());*/
    ...
    for (Track track : elements) {
      ...
      try {
        job = service.startTranscription(mediaPackage.getIdentifier().toString(), track, language, model,
                  responseFormat);
