...
define(["jquery", "underscore", "backbone", "engage/core"], function($, _, Backbone, Engage) {
    ...
    function getDownloadList(model) {
      ...
      _.each(model.get('attachments'), function(item) {
          if (item.type.indexOf('notes') > -1) {
              list.push(item);
          }
      });

      list.map(function(item) {
        if (item.url.substring(0, 7) === '/engage') {
          item.url = '/static/mh_default_org' + item.url;
          console.log('setting', item.url);
        }

        if (item.url.substring(0, 8) === '/static/') {
          item.url = '/' + item.url.split('/').slice(3).join('/');
        }

        item.fileExtension = getFileExtension(item.url);
